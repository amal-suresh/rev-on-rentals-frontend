...
function Home() {
  ...
  const fetchChat = async (token) => {
    try {
      setLoadingChat(true)
      const { data } = await axios.get(`${userApi}fetchIndividualChat`, {
  ...
  const fetchBikes = async () => {
    try {
      ...
      const { data } = await axios.get(`${userApi}ourFleet`)
      if (data.success) {
        setBikes(data.data)
      } else {
  ...
  useEffect(() => {
    if (user?.token) fetchChat(user.token)
    fetchBikes()
  }, [user])
  ...
  return (
    <div className="flex flex-col max-w-[1600px]">
      ...
      <div className="bg-black">
        <p className="text-center font-bold text-white text-[2.2rem] mt-10 font-passion tracking-wider">
          OUR FLEET
        </p>
          ...
          >
                    ...
                    >
                      ...
                      <div className="flex justify-end w-full">
                        <p className="text-[8px] font-semibold italic">
                          10:00 am {msg.sender === 'User' && <span className="pl-1 text-blue-800">&#10003;&#10003;</span>}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex items-center justify-center">
              <input
                ...
                value={message}
                className="w-[75%] text-sm focus:outline-none border border-slate-300 px-3 py-2 rounded-l-lg"
                type="text"
                placeholder="Ask Something?"
              />
              <button
                onClick={handleSendMessage}
                className="w-[20%] bg-black flex justify-center text-yellow-300 font-bold py-2 rounded-r-lg"
              >
                Send
              </button>
            </div>
